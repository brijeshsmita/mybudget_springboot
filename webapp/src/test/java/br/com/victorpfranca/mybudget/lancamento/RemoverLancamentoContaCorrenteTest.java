...
public class RemoverLancamentoContaCorrenteTest {
	...
	private LancamentoContaCorrente newLancamento(BigDecimal valor) {
		...
		return new LancamentoBuilder().data(date).inOut(inOutLancamentoInput).category(categoriaDespesa)
				.account(new Account().withId(1)).status(LancamentoStatus.NAO_CONFIRMADO).valor(valor)
