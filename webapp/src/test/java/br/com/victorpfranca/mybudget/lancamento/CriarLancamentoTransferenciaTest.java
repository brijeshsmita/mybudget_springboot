...
import br.com.victorpfranca.mybudget.lancamento.LancamentoBuilder;
...
public class CriarLancamentoTransferenciaTest {
	...
	public void shouldCreateLancamentoESaldos() {
		...
		assertEquals("Account", contaOrigemIdInput, lancamentos.get(0).getAccount().getId());
		assertNull("Account Origem", ((LancamentoContaCorrente) lancamentos.get(0)).getContaOrigem());
		...
		assertEquals("Account", contaDestinoIdInput, lancamentos.get(1).getAccount().getId());
		assertEquals("Account Origem", contaOrigemIdInput,
				((LancamentoContaCorrente) lancamentos.get(1)).getContaOrigem().getId());
		assertNull("Account Destino", ((LancamentoContaCorrente) lancamentos.get(1)).getContaDestino());
	...
	private LancamentoContaCorrente newLancamento() {
		...
		lancamento.setAccount(contaOrigem);
