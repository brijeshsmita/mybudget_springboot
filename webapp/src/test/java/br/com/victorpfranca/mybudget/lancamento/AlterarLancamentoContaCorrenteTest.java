...
public class AlterarLancamentoContaCorrenteTest {
	...
	private LancamentoContaCorrente newLancamento() {
		...
		return new LancamentoBuilder().data(dataLancamentoInput).inOut(inOutLancamentoInput).category(category)
				.account(contaBanco).status(LancamentoStatus.NAO_CONFIRMADO).valor(valorLancamentoInput)
