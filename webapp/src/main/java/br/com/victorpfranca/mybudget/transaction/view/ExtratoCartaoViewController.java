...
public class ExtratoCartaoViewController implements Serializable {
	...
	public void carregarDadosTelaListagem() {
		...
		this.transactions = lancamentoService.carregarExtratoCartaoMensal(filtroMes.getAno(), filtroMes.getMes(),
				filtroConta, filtroCategoria, BigDecimal.ZERO);
	}

	public void excluir(CreditCardInvoiceTransactionItem lancamento) {
		if (lancamento != null) {
			lancamentoService.removeLancamentoCartao((CreditCardTransaction) lancamento.getLancamentoCartao());
			carregarDadosTelaListagem();
		}
	}
	...
	public void excluirSerie(Transaction transaction) {
		if (transaction != null) {
			lancamentoService.removeSerieLancamentoCartao(transaction.getSerie());
			carregarDadosTelaListagem();
		}
	}
