...
import static br.com.victorpfranca.mybudget.orcamento.SaldoOrcadoAcumuladoMes.FIND_ALL;
import static br.com.victorpfranca.mybudget.orcamento.SaldoOrcadoAcumuladoMes.FIND_BY_DESPESA_UNTIL_MONTH;
import static br.com.victorpfranca.mybudget.orcamento.SaldoOrcadoAcumuladoMes.FIND_BY_RECEITA_UNTIL_MONTH;
...
@NamedQueries({
		@NamedQuery(name = FIND_ALL, query = "SELECT o FROM SaldoOrcadoAcumuladoMes o WHERE o.usuario.id = :user"),
		@NamedQuery(name = FIND_BY_DESPESA_UNTIL_MONTH, query = "SELECT o FROM SaldoOrcadoAcumuladoMes o where o.usuario.id = :user AND CONCAT(to_char(ano, 'FM9999'),to_char(mes, 'FM09')) <= CONCAT(to_char(:ano, 'FM9999'),to_char(:mes, 'FM09')) and inOut = '1' ORDER BY ano DESC, mes DESC"),
		@NamedQuery(name = FIND_BY_RECEITA_UNTIL_MONTH, query = "SELECT o FROM SaldoOrcadoAcumuladoMes o where o.usuario.id = :user AND CONCAT(to_char(ano, 'FM9999'),to_char(mes, 'FM09')) <= CONCAT(to_char(:ano, 'FM9999'),to_char(:mes, 'FM09')) and inOut = '0' ORDER BY ano DESC, mes DESC") })
